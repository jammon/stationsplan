...
Der aktuelle Datenbestand soll auf den Staging-Server übertragen werden.

- Auf Production:
  - Softwarestand auf production ermitteln
  - Data-Dump machen:

        ```
        python3.6 manage.py dumpdata --exclude=auth --exclude=contenttypes | gzip>`date +%Y%m%d`.json.gz
        ```

  - Daten runter laden: `scp stationsplan:stationsplan/`date +%Y%m%d`.json.gz .`
- Auf Staging:
